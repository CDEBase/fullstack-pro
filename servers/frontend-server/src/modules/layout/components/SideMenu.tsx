...
const getImageUrl = (picture) => {
	return (
		picture ||
		"data:image/png;base64,${new Identicon(Base64.encode('myawsomestringbebe'), 420).toString()}"
	);
};
...
export class SiderMenu extends React.PureComponent<
	ISiderMenu.Props,
	ISiderMenu.State
> {
	private menus;
	...
	private getAvatar(menu) {
		...
		return (
			<span
				...
				id={!user || user.isTest ? `cde-user-placeholder` : 'cde-user'}
			>
				<div
					style={{ marginRight: '7px' }}
					data-src={getImageUrl(user.picture)}
				>
					{user.nickname || 'Guest'}
				</div>{' '}
	...
	public render() {
		...
		return (
			...
			>
				{this.getLogo(
					(this.menus.filter((menu) => menu.position === IMenuPosition.LOGO) ||
						[])[0]
				)}
				<div className={styles.grow}>
					...
					>
						{this.getNavMenuItems(
							this.menus.filter(
								(menu) => menu.position === IMenuPosition.MIDDLE
							)
						)}
					</Menu>
				...
				>
					{this.getNavMenuItems(
						this.menus.filter((menu) => menu.position === IMenuPosition.BOTTOM)
					)}
				</Menu>
			</Sider>
		);
