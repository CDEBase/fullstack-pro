...
const modulenameExtra = process.env.MODULENAME_EXTRA
	? `${process.env.MODULENAME_EXTRA}|`
	: '';
const modulenameRegex = new RegExp(
	`node_modules(?![\\\\/](${modulenameExtra}@sample-stack)).*`
);
...
const config = {
	...
	module: {
		rules: [
			{ test: /\.mjs$/, include: /node_modules/, type: 'javascript/auto' },
			{
				...
				use: {
					loader: 'url-loader',
					options: { name: '[fullhash].[ext]', limit: 100000 },
				},
			},
			{
	...
	devServer: {
		...
		devMiddleware: {
			publicPath: '/',
			writeToDisk: (pathname) =>
				/(assets.json|loadable-stats.json)$/.test(pathname),
		},
		...(buildConfig.__SSR__
			? {
					proxy: {
