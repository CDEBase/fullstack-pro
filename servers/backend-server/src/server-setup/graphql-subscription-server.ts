...
export class GraphqlSubscriptionServer {
	...
	constructor(
		...
		private requestOptions: Partial<GraphQLServerOptions<any>> = Object.create(
			null
		)
	) {
		this.logger = this.moduleService.logger.child({
			className: 'GraphqlSubscriptionServer',
		});
	}
		...
		this.subscriptionServer = SubscriptionServer.create(
			{
				...
				onConnect: async (
					...
					try {
						this.logger.debug(
							`Subscription client connected using built-in SubscriptionServer.`
						);
						const pureContext = await this.moduleService.createContext(
							connectionParams,
							webSocket
						);
						const contextServices = await this.moduleService.serviceContext(
							connectionParams,
							webSocket
						);
						const context = {
