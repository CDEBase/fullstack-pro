...
const infraModule = ({ broker, pubsub, mongoClient, logger }) =>
	new ContainerModule((bind: interfaces.Bind) => {
		...
		bind(CommonType.ENVIRONMENT).toConstantValue(
...
export class StackServer {
	...
	public async initialize() {
		...
		const InfraStructureFeature = new Feature({
			...
			createServiceFunc: (container) => ({
				moleculerBroker: container.get(CommonType.MOLECULER_BROKER),
			}),
		...
		this.serviceContainer = await allModules.createContainers({
			...settings,
			mongoConnection: mongoClient,
		});
		const createServiceContext = allModules.createServiceContext({
			...settings,
			mongoConnection: mongoClient,
		});
		...
		if (customWebsocketEnable) {
			this.multiPathWebsocket = new WebsocketMultiPathServer(
				serviceBroker,
				redisClient,
				customWebsocket
			);
			this.httpServer = this.multiPathWebsocket.httpServerUpgrade(
				this.httpServer
			);
		}
	...
	public async start() {
		if (config.NODE_ENV === 'development') {
			await Promise.all([
				this.mainserviceBroker.start(),
				this.microserviceBroker.start(),
			]);
		} else {
