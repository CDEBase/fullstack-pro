...
const createActivityServiceFunc = (container: interfaces.Container) => {
	...
	if (environment === 'development') {
		return {
			activityService: container.get<IActivityCollector>(
				TYPES.ActivityCollector
			),
		};
	} else {
		return {
			activityService: container.getNamed<IActivityCollector>(
				TYPES.ActivityCollector,
...
export default new Feature({
	createContainerFunc: config.isDevelopment
		? [activityModule]
		: [activityModule, activityModuleNats],
