...
function deleteModule(logger, moduleName, location) {
	...
	if (fs.existsSync(modulePath)) {
		...
		// get index file path
		const indexFullFileName = fs
			.readdirSync(modulesPath)
			.find((name) => name.search(/index/) >= 0);
		...
		const appModulesWithoutDeleted = appModules
			.split(',')
			.filter((appModule) => appModule.trim() !== moduleName);
		...
		const contentWithoutDeletedModule = indexContent
			...
			.replace(
				appModuleRegExp,
				`Module(${appModulesWithoutDeleted.toString().trim()})`
			)
			// remove module import
			.replace(
				RegExp(`import ${moduleName} from './${moduleName}';\n`, 'g'),
				''
			);
		...
		logger.info(
			chalk.red(`✘ Module ${location} location for ${modulePath} not found!`)
		);
	}
}
